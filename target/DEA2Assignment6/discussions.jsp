...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/discussions.SCSS">
...
<body>
<div class="header">
    <a href="#" class="logo">Discussions</a>
    <div class="header-right">
        <!--        <a class="" href="#home">Home</a>-->
        <c:if test="${user.amILogin == true}">
            <a href="newPost.html">Add post</a>
        </c:if>
        <c:if test="${user.amILogin == true}">
            <a href="logout.jsp">logout</a>
        </c:if>
        <c:if test="${user.amILogin == false}">
            <a href="index.jsp">exit</a>
        </c:if>

    </div>
</div>
<c:forEach var="discuss"
           items="${discussion.findAll('jdbc:postgresql://localhost:5432/f_db?user=postgres&password=1234')}">
    <div class="card">
        ...
        <div class="card-text">
            ...
            <div class="title-total">
                <div class="title">user_id: <c:out value="${discuss.user_id}"/></div>
                <h2><c:out value="${discuss.title}"/></h2>

                <div class="desc"><c:out value="${discuss.subtext}"/></div>
                <div class="actions">
                    <%
                        System.out.println(user.getId() + "and" + user.isAmILogin());
                    %>
                    <c:if test="${discussion.likedByMeOrNot('jdbc:postgresql://localhost:5432/f_db?user=postgres&password=1234', discuss.id, user.id == null ? 0 : user.id ) == false && user.amILogin == true}">

                        <form action="addLike.jsp" method="post">
                            <button name="btn">
                                <input type="hidden" name="discuss_id" value="<c:out value='${discuss.id}' />"/>
                                <input type="hidden" name="user_id" value="<c:out value='${user.id}' />"/>
                                <c:out value="${discussion.findDiscussionLikes('jdbc:postgresql://localhost:5432/f_db?user=postgres&password=1234', discuss.id)}"/>
                                <i class="fa fa-heart-o" aria-hidden="true"></i></button>

                        </form>
                    </c:if>

                    <c:if test="${discussion.likedByMeOrNot('jdbc:postgresql://localhost:5432/f_db?user=postgres&password=1234', discuss.id, user.id == null ? 0 : user.id ) == true || user.amILogin == false}">
                        <button>
                            <c:out value="${discussion.findDiscussionLikes('jdbc:postgresql://localhost:5432/f_db?user=postgres&password=1234', discuss.id)}"/>
                    ...
                    <button>
                        <c:out value="${discussion.findDiscussionComments('jdbc:postgresql://localhost:5432/f_db?user=postgres&password=1234', discuss.id)}"/>
